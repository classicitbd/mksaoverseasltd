            ...
            <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" aria-describedby="example1_info">
                <thead>
                    <tr>
					  ...
					  <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Gallery Image</th>
                ...
                <tbody> 
                    ...
                    <tr class="odd">
                        ...
                        <td><img src="{{asset('public/img/'.$gallerycategories->image)}}" height="100px" width="150px" alt=""></td>
                        <td>{{$gallerycategories-> title}}</td>
                        <td>{{$gallerycategories-> gc_name}}</td>
			...
			<div class="modal-body">
				<form action="{{route('gallerycategories.store')}}" method="post" enctype="multipart/form-data">
					...
					<div class="row">
						...
						<div class="col-sm-12">
    						<div class="input-group mb-4">
    							<label class="col-form-label">Gallery Group:&nbsp;</label>
    							<select id="txtGroup" class="form-control" name="txtGroup" required>
    								<option selected><------------Select Group----------------></option>
    								<option value="1">Company Photo</option>
    								<option value="2">Seminer</option>
    								<option value="3">Occational</option>
		...
		$(document).on('click','#editgallerycategories',function(){
			...
			$.ajax({
				...
				success:function(response){
					...
					$('#eGallerycategories').val(response.gallerycategories.gc_name);
					$('#eGroup').val(response.gallerycategories.g_group);
					$('#eTitle').val(response.gallerycategories.title);
					$('#eURL').val(response.gallerycategories.url);
					$("#eFilephoto").html(
                        `<img src="public/img/${response.gallerycategories.image}" width="100" class="img-fluid img-thumbnail">`);
